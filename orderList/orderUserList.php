...
$sql = $db_host->prepare("SELECT * FROM order_list WHERE order_valid=1 ORDER BY order_id DESC");
$sql->execute();
                      ...
                      <table class="table table-hover text-nowrap">
                        <thead class="table-info">
                          <tr class="headings">
                            ...
                            <th class="column-title">訂單金額 </th>
                            <th class="column-title">訂購人 </th>
                            <th class="column-title">訂單日期</th>
                            <th class="column-title">地址 </th>
                            <th class="column-title">訂單狀態 </th>
                            <th class="column-title">付款狀態 </th>
                            <th class="column-title no-link last"><span class="nobr">工作</span>
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              <td class="align-middle"> <?= $row["order_amount"] ?> </td>
                              <td class="align-middle"> <?= $row["order_id"] ?> </td>
                              <td class="align-middle"> <?= $row["order_date"] ?> </td>
                              <td class="align-middle"> <?= $row["order_address"] ?> </td>
                              <td class="align-middle text-center">
                                <?php if ($row["order_status"] == 0) { ?>
                                  <div class="text-red text-bold">未出貨</div>
                                <?php } elseif ($row["order_status"] == 1) { ?>
                                  <span class="text-blue text-bold">運送中</span>
                                <?php } else { ?>
                                  <span class="text-bold">已出貨</span>
                                <?php } ?>
                              </td>
                              <td class="align-middle text-center">
                                <?php if ($row["order_payment_status"] == 0) { ?>
                                  <span class="text-red text-bold">未付款</span>
                                <?php } else { ?>
                                  <span class="text-bold">已付款</span>
                                <?php } ?>
                              </td>
                              <td class="last align-middle text-center">
                                <a href="orderUser.php?order_id=<?= $row["order_id"] ?>" class="btn btn-app bg-info"><i class="fas fa-edit"></i> 修改 </a>
                                <a href="orderUserDelete.php?order_id=<?= $row["order_id"] ?>" class="btn btn-app bg-danger"> <i class="fas fa-trash-alt"></i> 刪除</a>
                              </td>
                            </tr>
                          <?php }  ?>
                        <?php
