...
$sql = $db_host->prepare("SELECT * FROM board ORDER BY board_date DESC");
$sql->execute();
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item">
              <a href="boardUserList.php" class="nav-link  active">
            ...
            <li class="nav-item">
              ...
              <ul class="nav nav-treeview">
                ...
                <li class="nav-item">
                  <a href="../app/views/views/member-ask.php" class="nav-link">
              ...
              <div class="card p-2">
                ...
                <table class="table table-bordered table-hover mt-2">
                  <thead class="table-info">
                    <tr class="headings">
                      <td class="column-title">留言者</td>
                      <td class="column-title">留言板編號</td>
                      <td class="column-title">留言者名稱</td>
                      <td class="column-title">留言板內容</td>
                      <td class="column-title">檢舉</td>
                      <td class="column-title">留言時間</td>
                      <td class="column-title">備註</td>
                    </tr>
                  </thead>
                  <tbody id="target">
  ...
  <script>
    ...
    $.ajax({
      ...
      .done(function(data) {
        ...
        data.forEach((user) => {
          content += `
                    <tr id="item${user.board_member_id}">
                        <td>${user.board_member_id}</td>
                        <td>${user.board_id}</td>
                        <td>${user.reply_member_id}</td>
                        <td>${user.board_content}</td>
                        <td>${user.board_report}</td>
                        <td>${user.board_date}</td>
                        <td>
                            <button data-board_member_id="${user.board_member_id}" class="btn btn-app bg-danger delete-btn "> <i class="fas fa-trash-alt"></i>刪除</button>
                        </td>
                    </tr>
                    `
        })
    ...
    $("#target").on("click", ".delete-btn", function() {
      console.log("test");
      let targetId = $(this).data("board_member_id");
      Swal.fire({
        icon: 'warning',
        text: '確認刪除?',
        ...
        if (result.isConfirmed) {
          ...
          $.ajax({
              method: "POST",
              url: "boardDelUsers.php",
              data: {
                board_member_id: targetId
              },
              dataType: "json"
            })
            .done(function(data) {
